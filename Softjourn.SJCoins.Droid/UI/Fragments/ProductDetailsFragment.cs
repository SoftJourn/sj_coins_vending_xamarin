...
namespace Softjourn.SJCoins.Droid.UI.Fragments
{
    public class ProductDetailsFragment : BottomSheetDialogFragment
    {
        ...
        public static ProductDetailsFragment GetInstance(Product product)
        {
            ...
            var serializedObj = JsonConvert.SerializeObject(product);
            bundle.PutString(ArgProduct, serializedObj);
            var fragment = new ProductDetailsFragment { Arguments = bundle };
        ...
        public override void OnCreate(Bundle savedInstanceState)
        {
            ...
            _product = JsonConvert.DeserializeObject<Product>(Arguments.GetString(ArgProduct));
        }

        public override void OnViewCreated(View contentView, Bundle savedInstanceState) => base.OnViewCreated(contentView, savedInstanceState);
        ...
        /// Calls Purchase method on Activity to which is attached to
        ...
        /// Sets appropriate image to favorite imageView
        /// and changes its tag
        /// </summary>
        ...
        /// Sets appropriate image to favorite imageView
        /// and changes its tag
        /// Calls TrigFavorite method on Activity to which is attached to
        /// and transfer Product object to it.
        /// </summary>
        private void HandleOnFavoriteClick()
        {
            if (!(bool)_favorites.Tag)
            {
                if (Activity.LocalClassName.Contains("MainActivity"))
                {
                    ...
                    ((ShowAllActivity)Activity).TrigFavorite(_product);
            }
            else
            {
                if (Activity.LocalClassName.Contains("MainActivity"))
